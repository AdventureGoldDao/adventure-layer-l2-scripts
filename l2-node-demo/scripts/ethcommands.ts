...
export const sendL1Command = {
  ...
  handler: async (argv: any) => {
    console.log("transfer-l1-erc20");
    argv.provider = new ethers.providers.WebSocketProvider(argv.l1url);
    const account = namedAccount(argv.from).connect(argv.provider);
    const tokenContract = new ethers.Contract(argv.token, ERC20.abi, account);

    const code = await argv.provider.getCode(argv.token);
    if (code === "0x") throw new Error("Invalid token address");
    ...
    if (balance.lt(amount)) {
      throw new Error(`Insufficient balance: ${ethers.utils.formatUnits(balance, decimals)} < ${argv.amount}`);
    }

    try {
      await (await tokenContract.transfer(
